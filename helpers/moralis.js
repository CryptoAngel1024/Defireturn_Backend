...
async function retryMoralisFunc(moralisFunc, args, retryCnt = -1) {
  ...
  while (retryCnt == -1 || cnt++ < retryCnt) {
    try {
      ...
      const delayTime = Number(2000 + 2000 * Math.random()).toFixed(0);
      console.log(
        `Retrying Moralis func after ${delayTime} milisecond for`
      );
      await new Promise((resolve) => setTimeout(resolve, delayTime));
    }
  }
...
module.exports = {
  STATE,
  init,
  getCurrentBlockNumber: async (...args) => { return await retryMoralisFunc(getCurrentBlockNumber, args, maxRetryCnt) },
  getTokenBalances: async (...args) => { return await retryMoralisFunc(getTokenBalances, args, maxRetryCnt) },
  getTokenTransfers: async (...args) => { return await retryMoralisFunc(getTokenTransfers, args, maxRetryCnt) },
  getTransactions: async (...args) => { return await retryMoralisFunc(getTransactions, args, maxRetryCnt) },
  getTokenMetadata: async (...args) => { return await retryMoralisFunc(getTokenMetadata, args, maxRetryCnt) },
  getTokenPrice: async (...args) => { return await retryMoralisFunc(getTokenPrice, args, maxRetryCnt) },
  getAssets,
  getDebankValue,
  getCovalentPrice,
  latestBlockHeight,
